...
interface PublicMediaContextType {
...
const PublicMediaContext = createContext<PublicMediaContextType | undefined>(
  undefined
);

export const usePublicMedia = () => {
  const context = useContext(PublicMediaContext);
  if (context === undefined) {
    throw new Error("usePublicMedia must be used within a PublicMediaProvider");
  }
  return context;
};

interface PublicMediaProviderProps {
  children: ReactNode;
}
...
export const PublicMediaProvider = ({ children }: PublicMediaProviderProps) => {
  ...
  const fetchPublicMedia = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/media?isActive=true&sortBy=createdAt&sortOrder=desc`
      );
  ...
  const value: PublicMediaContextType = {
    mediaItems,
    loading,
    error,
    refreshMedia,
  ...
  return (
    <PublicMediaContext.Provider value={value}>
      {children}
    </PublicMediaContext.Provider>
  );
};

// Alternative: Simple hook without context for direct use in ShowreelPage
export const useMediaItems = () => {
  ...
  return { mediaItems, loading, error, refetch: fetchMedia };
};
