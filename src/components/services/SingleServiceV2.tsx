...
const SingleServiceV2 = ({ service }: { service: DataType }) => {
  ...
  const { activeIndex, hoveredIndex, handleMouseEnter, handleMouseLeave } =
    useHoverEffects();

  return (
    <li
      className={`hover-active-item ${
        activeIndex === hoveredIndex ? "active" : ""
      }`}
      onMouseEnter={() => handleMouseEnter(id)}
      onMouseLeave={handleMouseLeave} // Call without index
    >
      <Link to="/services" className="service-hover-item">
        ...
        <div
          className={`service-hover-wrapper service-hover-wrapper-${id}`}
          style={{
            opacity: 0,
            position: "absolute",
